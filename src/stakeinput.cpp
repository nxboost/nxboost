...
//!NXB Stake
bool CNXBStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CNXBStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CNXBStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CNXBStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CNXBStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CNXBStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CNXBStake::GetUniqueness()
{
    //The unique identifier for a NXB stake is the outpoint
...
CBlockIndex* CNXBStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
