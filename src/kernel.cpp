...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZNXBStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zNXB fails context checks", __func__);
    } else {
        ...
        CNXBStake* NXBInput = new CNXBStake();
        NXBInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(NXBInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zNXB is checked above in ContextualCheckZerocoinStake()
