...
CzNXBWallet::CzNXBWallet(std::string strWalletFile)
{
        ...
        if (walletdb.ReadZNXBSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNXBSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zNXB seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zNXB wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNXBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzNXBWallet::Lock()
{
    seedMaster = 0;
}

void CzNXBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNXBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzNXBWallet::LoadMintPoolFromDB()
{
...
void CzNXBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNXBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNXBWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzNXBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
...
uint512 CzNXBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNXBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNXBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNXBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
