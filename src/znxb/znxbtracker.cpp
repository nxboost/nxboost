...
CzNXBTracker::CzNXBTracker(std::string strWalletFile)
{
...
CzNXBTracker::~CzNXBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNXBTracker::Init()
{
...
bool CzNXBTracker::Archive(CMintMeta& meta)
{
...
bool CzNXBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNXBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNXBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNXBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzNXBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNXBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNXBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNXBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNXBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNXBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNXBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNXBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNXBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNXBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNXBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNXBWallet* zNXBWallet)
{
    bool iszNXBWalletInitialized = (NULL != zNXBWallet);
    CMintMeta meta;
    ...
    if (! iszNXBWalletInitialized)
        zNXBWallet = new CzNXBWallet(strWalletFile);
    meta.isSeedCorrect = zNXBWallet->CheckSeed(dMint);
    if (! iszNXBWalletInitialized)
        delete zNXBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNXBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNXBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNXBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNXBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNXBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
