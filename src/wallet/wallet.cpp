        ...
        bool isPublicSpend)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zNXB public spend transaction
    ...
    if (vSelectedMints.empty()) {
        ...
        setMints = znxbTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
