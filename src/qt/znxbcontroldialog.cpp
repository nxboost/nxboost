    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                strReason = tr("Needs %1 more confirmations").arg(Params().Zerocoin_MintRequiredConfirmations() - nConfirmations);
            else if (model->getEncryptionStatus() == WalletModel::EncryptionStatus::Locked)
                strReason = tr("Your wallet is locked. Impossible to precompute or spend zNXB.");
            else if (!mint.isSeedCorrect)
                strReason = tr("The zNXB seed used to mint this zNXB is not the same as currently hold in the wallet");
            else
                strReason = tr("Needs %1 more mints added to network").arg(Params().Zerocoin_RequiredAccumulation());

            itemMint->setText(COLUMN_ISSPENDABLE, strReason);
        } else {
            itemMint->setText(COLUMN_ISSPENDABLE, tr("Yes"));
        }
    }
...
void ZNxbControlDialog::updateLabels()
{
    ...
    ui->labelzNXB_int->setText(QString::number(nAmount));
