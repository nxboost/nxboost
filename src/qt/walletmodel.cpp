...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 NXB.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzNXBReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzNXBReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzNXBReset.connect(boost::bind(NotifyzNXBReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzNXBReset.disconnect(boost::bind(NotifyzNXBReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->znxbTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
