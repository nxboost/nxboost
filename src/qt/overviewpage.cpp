...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::NXB)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sNXBPercentage, QString& szNXBPercentage)
{
    int nPrecision = 2;
    ...
    szNXBPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sNXBPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount NXBAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // NXB Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = NXBAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // NXB labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, NXBAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelNXBPercent->setText(sPercentage);
    ui->labelzNXBPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zNXB.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in nxboost.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in nxboost.conf");
    }
    ...
    // zNXB
    bool showzNXBAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzNXBUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzNXBImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzNXBAvailable);
    ui->labelzBalanceMatureText->setVisible(showzNXBAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzNXBUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzNXBUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzNXBImmature);
    ui->labelzBalanceImmatureText->setVisible(showzNXBImmature);

    // Percent split
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelNXBPercent->setVisible(showPercentages);
    ui->labelzNXBPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("NXB")
    updateDisplayUnit();
}
